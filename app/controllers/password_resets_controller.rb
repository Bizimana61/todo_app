class PasswordResetsController < ApplicationController
  ...
  def update
    if params[:user][:password].empty?
      ...
      # Regenerate session token to invalidate all other sessions
      @user.regenerate_session_token

      # Reset current session and log in
      reset_session
      ...
      redirect_to todos_path, notice: "Password has been reset successfully! All other devices have been logged out."
    else
      render :edit, status: :unprocessable_entity
    end
  end
